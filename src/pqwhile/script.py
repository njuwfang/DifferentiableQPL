...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default='qprog',
        help='Name for operation in output Q# file.\nDefault: qprog'
    )
    parser.add_argument(
        '--method', '-m',
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default='-1',
        help='Specify the first DIFF_NUM paramters to perform differentiation.\nDefault: -1, for all parameters'
    )
    args = parser.parse_args()
    with open(args.pqwhile_file, 'r') as qprog_file:
        qprog = qprog_file.read()
        parse2qsharp(qprog, args.outfile, args.name, args.method, args.diff_num)